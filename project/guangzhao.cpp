#include "guangzhao.h"
#include "ui_guangzhao.h"
#include <QSqlQuery>
#include <QPainter>

Guangzhao::Guangzhao(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Guangzhao)
{
    ui->setupUi(this);
...
void Guangzhao::changeEvent(QEvent *e)
{
...
void Guangzhao::shuju()   //读取数据
{
    ...
    while(query.next())
    {
        w[i]=query.value(2).toFloat();
        y[i]=180-8*w[i];
        i++;
    }
}

void Guangzhao::paintEvent(QPaintEvent *event)
{
   ...
   QPointF(250,w5)
      ...
      painter.drawPolyline(guangqiang,6);
...
void Guangzhao::on_pushButton_clicked()
{
    close();
}
